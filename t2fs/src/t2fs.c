    ...
    while(aux != NULL){
        if(!strcmp(filename, aux->dirent.name)){
           for(j = 0; j < MAX_ARQUIVOS_ABERTOS; j++){
            ...
            if(DEBUG_MODE) 
                printf("\033[0;32mDeletado o arquivo %s (verificar se foi de fato liberado devido aos hardlinks)\n\033[0m", filename);
...
int hln2(char *linkname, char *filename){
	...
	struct t2fs_inode iNodeFile;
    readINodeM(iNodeDoApontado, &iNodeFile);
    iNodeFile.RefCounter += 1;
    writeINodeM(iNodeDoApontado, iNodeFile);
